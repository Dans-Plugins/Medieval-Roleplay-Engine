...
public class AsyncPlayerChatEventHandler {
    ...
    public void handle(AsyncPlayerChatEvent event) {
        if (MedievalRoleplayEngine.getInstance().playersSpeakingInLocalChat.contains(event.getPlayer().getUniqueId())) {
            sendMessageToPlayersWithinDistance(event.getPlayer(), ChatColor.GRAY + "" + String.format("%s: \"%s\"", MedievalRoleplayEngine.getInstance().utilities.getCard(event.getPlayer().getUniqueId()).getName(), event.getMessage()), 25);
