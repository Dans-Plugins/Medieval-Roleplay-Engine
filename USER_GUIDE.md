...
- **`rp.rphelp` is accepted but unregistered.** `/rphelp` accepts it as an alternative to `rp.help`. It is not registered in `plugin.yml`, so it must be granted explicitly through a permissions plugin to have any effect. It is retained only so that servers which already granted it keep working; `rp.help` is the node to grant.
